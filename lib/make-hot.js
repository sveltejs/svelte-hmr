...
const createMakeHot = ({ resolveAbsoluteImport }) => ({
  ...
  const adapterImport = posixify(
    hotOptions.adapter || resolveAbsoluteImport('runtime/' + defaultAdapter)
  )

  const hotApiImport = posixify(
    hotOptions.hotApi || resolveAbsoluteImport('runtime/' + defaultHotApi)
  )
