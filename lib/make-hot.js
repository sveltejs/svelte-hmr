...
const renderApplyHmr = ({
  ...
  cssId,
  nonCssHash,
  hotOptions: { injectCss, compatVite },
  hotOptionsJson,
  hotApiImport,
  adapterImport,
  importAdapterName,
  ...
  `${imports.join(';')};${`
    if (${compatVite ? '' : `${meta} && `}${meta}.hot) {
      ...
      $2 = ${globalName}.applyHmr({
        ...
        cssId: ${json(cssId)},
        nonCssHash: ${json(nonCssHash)},
        ignoreCss: ${json(ignoreCss)},
      });
    }
  `
    .split('\n')
...
const createMakeHot = ({ resolveRuntimeImport }) => ({
  ...
  //
  const resolveAdapterImport = ({ native, absoluteImports }, adapterOpt) => {
    if (adapterOpt) {
      return posixify(adapterOpt)
    }
    ...
    return posixify(resolveRuntimeImport(absoluteImports, file))
  }

  const resolveHotApiImport = ({ absoluteImports }, hotApiOpt) => {
    if (hotApiOpt) {
      return posixify(hotApiOpt)
    }
    return posixify(resolveRuntimeImport(absoluteImports, defaultHotApi))
  }

  const adapterImport = resolveAdapterImport(hotOptions, hotOptions.adapter)

  const hotApiImport = resolveHotApiImport(hotOptions, hotOptions.hotApi)
